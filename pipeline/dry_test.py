...
def main():
    print("--- Starting Dry Run Test ---")  # <-- CHANGE
    ...
    # ... (all the file path logic stays the same) ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    ...
    # Slice DF for test
    df_reviews = df_reviews.head(5)  # Only process the first 5 reviews
    # --------------------------------------------------

    print(f"'Live' review file loaded. PROCESSING FIRST 5 ROWS FOR TEST.")
    ...
    for _, review in tqdm(df_reviews.iterrows(), total=len(df_reviews), desc="Processing Reviews"):
        ...
        # Add a "mock" result instead
        llm_result = {"theme": "MOCK_THEME", "sentiment": "MOCK_SENTIMENT"}
        # ----------------------------------------
    ...
    print("\nPipeline (DRY RUN) complete! Generating priority dashboard...")

    df_final = pd.DataFrame(results)
    df_final = df_final.sort_values(by=['CLTV', 'Churn_Probability'], ascending=[False, False])

    df_final.to_csv(output_file_path, index=False)

    print(f"--- Success! ---")
    print(f"Priority dashboard (TEST) saved to {output_file_path}")
    print("\n--- Top 5 Customers (TEST) ---")
    print(df_final.head())
