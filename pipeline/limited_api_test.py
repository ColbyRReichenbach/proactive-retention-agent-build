...
def main():
    # Let's call this a LIMITED RUN to be clear
    print("--- Starting Limited Run Test (10 rows) ---")
    ...
    # Change the output file name so we don't overwrite anything
    output_file_path = os.path.join(script_dir, "analyst_priority_list_LIMITED_TEST.csv")
    ...
    # Slice the DataFrame to only process the first 10 rows
    df_reviews = df_reviews.head(10)

    print(f"'Live' review file loaded. PROCESSING FIRST {len(df_reviews)} ROWS FOR LIMITED TEST.")
    ...
    for _, review in tqdm(df_reviews.iterrows(), total=len(df_reviews), desc="Processing Reviews"):
        ...
        # Look up customer in the "warehouse"
        try:
            customer_data = df_warehouse.loc[customer_id]
        except KeyError:
            print(f"⚠️ Warning: CustomerID {customer_id} not found in data warehouse. Skipping.")
            continue
    ...
    print("\nPipeline (LIMITED RUN) complete! Generating priority dashboard...")

    # Convert list of results into our final DataFrame
    df_final = pd.DataFrame(results)

    # Create the unified Priority Score
    df_final['Priority_Score'] = df_final['Churn_Probability'] * df_final['CLTV']

    # Sort by our new score, highest first
    ...
    print(f"--- Success! ---")
    print(f"Priority dashboard (LIMITED TEST) saved to {output_file_path}")
    print("\n--- Top Customers (LIMITED TEST) ---")
    print(df_final.head())
